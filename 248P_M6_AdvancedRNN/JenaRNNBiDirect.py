...
def build_model(data):
    #generate model with a Bidiractional RNN using GRU
    #ouput layer is a single neuron in dense layer without any activation 
    #This is because we need a linear output(regression problem)
    #rmsprop optimizer with mean absolute error as loss function
    model = Sequential()
    model.add(layers.Bidirectional(layers.GRU(32), input_shape=(None, data.shape[-1])))
